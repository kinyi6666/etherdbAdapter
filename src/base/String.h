...
#ifndef __EtherDB_String_H_
#define __EtherDB_String_H_
...
namespace EtherDB
{
	...
	template <class S>
	std::vector<S> split(S s, S delim, bool empty = false)
	{
		S  str = s;
		std::vector<S> ret;
		std::size_t pos = 0;
		...
		{
			S value;
			pos = isubstr(str, delim);
			if (pos != std::string::npos)
			{
				value = S(str.begin(), str.begin() + pos);
				str = S(str.begin() + pos + delim.length(), str.end());
			}
			else
			{
				value = S(str.begin(), str.end());
			}
			if (value != "")
			{
				ret.push_back(value);
